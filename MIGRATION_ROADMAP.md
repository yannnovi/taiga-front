...
3. **Validation de formulaire (checksley → Angular Reactive Forms)** — 🚧 **en cours**, le
   ...
   **Neuf formulaires/widgets migrés : `tgLbFeedback`, `tgLbCreateBulkIssues`,
   ...
   Détail dans `MIGRATION.md`. **`admin/project-profile.coffee` entièrement fait** (3 routes :
   "Project details", "Default Values", "Modules") - `AdminProjectProfileFormComponent`
   (formulaire éditable + variante lecture-seule projet archivé), `AdminProjectDefaultValuesFormComponent`,
   `AdminProjectModulesFormComponent`, `ProjectLogoComponent`, 3 wrappers `UpgradeComponent`
   ...
   (scope ambiant, drag-and-drop) et un contrôleur "Tags" séparé, jamais examinés.
   **`third-parties.coffee` entièrement fait** : les 4 formulaires d'intégration git
   (`ThirdPartyWebhookFormComponent` partagé, aucune validation réelle dans l'original
